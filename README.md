...
**Important Notes about `train_first_model.py`:**
- This is a **standalone demonstration script** for quick setup
- It's **specifically for the Adult Income dataset** (hardcoded)
- Perfect for first-time users to understand the workflow
- For other datasets (COMPAS, synthetic), use the dashboard or Python scripts instead
- The saved model can be loaded in the dashboard for monitoring

---

## 📦 Understanding Scripts and Datasets

### `train_first_model.py` - Purpose and Usage

**What it is:**
- A demonstration script showing the complete ML pipeline
- Hardcoded to use the **Adult Income dataset**
- Trains a baseline XGBoost model for monitoring demos

**When to use:**
- ✅ First time using the project
- ✅ Quick testing that all components work
- ✅ Creating a baseline model for dashboard demos
- ✅ Learning the workflow

**When NOT to use:**
- ❌ For COMPAS dataset (use dashboard or `load_dataset("compas")`)
- ❌ For custom training configs (use dashboard or `ModelTrainer` directly)
- ❌ For production workflows (use dashboard/API)

**What it does:**
1. Loads Adult Income (auto-downloads from OpenML if needed)
2. Preprocesses data
3. Trains XGBoost model
4. Evaluates and saves everything to `models/` directory

### Adding COMPAS Dataset

COMPAS is **already supported** in the codebase. To use it:

**Step 1:** Download the dataset
- Go to: https://github.com/propublica/compas-analysis
- Download: `compas-scores-two-years.csv`
- Place it in: `data/raw/compas-scores-two-years.csv`

**Step 2:** Use in dashboard or code
- **Dashboard**: Go to "Data Explorer" → Select "compas" → Load
- **Python**: `from src.data.load_data import load_dataset; load_dataset("compas")`

**Step 3:** Train with COMPAS
- Use the dashboard "Model Training" page, OR
- Use Python: Load data → Preprocess → Train with `ModelTrainer`
